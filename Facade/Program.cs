...
namespace Coding.Exercise
{
    ...
    public class MagicSquareGenerator
    {
        public List<List<int>> Generate(int size)
        {
            ...
            bool verified = false;
            List<List<int>> randomMatrix = new List<List<int>>();
            while (!verified)
            {
                randomMatrix = new List<List<int>>();
                for (int i = 0; i < size; i++)
                {
                    randomMatrix.Add(generator.Generate(size));
                }

                var splittedMatrix = splitter.Split(randomMatrix);
